...
def read_values(files):
    ...
    for f in file_list:
        try:
            values = np.load(f, allow_pickle=True).item()        
...
def define_param_mappings(values):
    """
    ...
    Args:
        values - all dictionaries, all organized as outlined above
    ...
    input_params, param_space = define_param_space(values[0])
    ...
    key = ['k']*(len(input_params)+3)

    for v in values:
        # across different input parameters
        # TODO check? assert v["input_params"].keys() == input_params, \
        #        "Error: Input parameter spaces don't match"

        for i in range(len(input_params)):
            kk = input_params[i]
            vv = str(v["input_params"][kk])
            key[i] = vv
            param_space[i].add(vv)

        # across different tracked output values
        # TODO check? assert (v["output_values"].keys() == quantities), \
        #        "Error: Output values don't match"
        
        for q in quantities:
            key[-3] = q
            
            # because D, and maybe M, can vary:
            _, M, D = v["output_values"][q].shape   
            ...
            for m in range(M):
                ...
                for d in range(D):
                    ...
                    param_mapping[tuple(key)] = \
                        v["output_values"][q][:,m,d]
        
        # + time
        time_mapping[tuple(key[:-3])] = v["time"]

    for i in range(len(param_space)):
        param_space[i] = list(param_space[i])
...
def get_all_combinations(headers, checkboxes):
    ...
    return plt_map, list(product(*param_list))
...
def update_args(headers, param_mapping, time_mapping, figsize, \
    ...
    figs, axs = plt.subplots(N_plots, 1, figsize=(figsize[0], \
...
def visualization_setup(widgets, files, figsize):
    """
    
    Setup for everything above.
